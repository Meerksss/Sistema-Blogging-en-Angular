...
export class BlogComponent {
  postBlog : IBlog = {titulo: '', imagen: '', descripcion: '', fecha: new Date()
  };
  ...
  crearPost() {
    ...
    this.postBlog = {titulo: '', imagen: '', descripcion: '', fecha: new Date() }
  }
  ...
  pintarPost() {
    ...
    this.arrayPosts.forEach (postBlog =>{
      ...
      <h5 class="card-title">${postBlog.titulo}</h5>
        <img src="${postBlog.imagen}" class="card-img-top" alt="${postBlog.titulo}">
        <div class="card-body">
          <p class="card-text">${postBlog.descripcion}</p>
          <p class="card-text"><small class="text-muted">Publicado el ${postBlog.fecha.toLocaleDateString()}</small></p>
        </div>
      </div>`;
    })
    return html;
